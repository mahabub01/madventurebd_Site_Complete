...
if($db->getData("post",array("id","title","description","image","categoryid","date1","name"), " where (title like '%".$_SESSION['search']."%' or description like '%".$_SESSION['search']."%')")){
...
foreach ($db->getData("post",array("id","title","description","image","categoryid","date1","name"), " where (title like '%".$_SESSION['search']."%' or description like '%".$_SESSION['search']."%')") as $key => $value) {
	
?>

<!--start post box-->
<div class="blog-post">
<img src="apps/postimages/<?php echo $value['id'].'_'.$value['image'];?>">
...
<form action="<?php appsConfig::URL('/search')?>" method="post">
	<input type="search" name="search" value="<?php echo $_SESSION['search'];?>">
...
<?php
$i = 1;
foreach ($db->getData("post",array("id","title","description","image","categoryid","date1","name")) as $key => $value) {
